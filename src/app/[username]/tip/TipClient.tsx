...
export default function TipClient({ username, tips }: Props) {
    ...
    <PageWrapper>
      <div style={{ padding: '24px 16px 40px' }}>
        ...
        <div style={{ textAlign: 'center', paddingTop: 32, paddingBottom: 32 }}>
          <div style={{ fontSize: 64, marginBottom: 12, display: 'block', animation: 'float 3s ease-in-out infinite' }}>
            💛
          </div>
          <div style={{ fontFamily: "'Syne', var(--font-syne), sans-serif", fontWeight: 800, fontSize: 26, marginBottom: 8 }}>
            Leave a Tip
          </div>
          ...
          {tips.map((tip, i) => (
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 16, color: COLORS.text }}>{tip.name}</div>
                  <div style={{ fontSize: 13, color: tip.color, marginTop: 2 }}>{tip.handle}</div>
                </div>
                {sent === tip.name
